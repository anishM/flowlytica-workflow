...
To configure Flowlytics - Workflow, you will need to set some environment variables. There is file called `.env` at the root directory for our main repo.
...
| `AP_CLOUD_AUTH_ENABLED`            | Turn off the utilization of Flowlytics - Workflow OAuth2 applications                                                                                                                                               | `false`                                                                        |                                                                        |
| `AP_DB_TYPE`                       | The type of database to use. (POSTGRES / SQLITE3)                                                                                                                                                                    | `SQLITE3`                                                                      |                                                                        |
| `AP_EXECUTION_MODE`                | You can choose between 'SANDBOX_PROCESS', 'UNSANDBOXED', 'SANDBOX_CODE_ONLY', 'SANDBOX_CODE_AND_PROCESS' as possible values. If you decide to change this, make sure to carefully read https://www.flowlytics.com/docs/install/architecture/workers                                                                | `UNSANDBOXED`                                                                  |                                                                        |
...
| `AP_INTERNAL_URL`                  | (BETA) Used to specify the SSO authentication URL.                                                                                 | None                                                                         | [https://demo.flowlytics.com/api](https://demo.flowlytics.com/api) |
| `AP_JWT_SECRET`                    | ❗️ Encryption key used for generating JWT tokens is a 32-character hexadecimal key. You can generate one using the following command: `openssl rand -hex 32`.                                                                                                                                                                     | None                                                                         | [https://demo.flowlytics.com](https://demo.flowlytics.com)         |
...
| `AP_PIECES_SOURCE`                 | `AP_PIECES_SOURCE`: `FILE` for local development, `DB` (default) to load from the database/registry. You can find more information about it in [Setting Piece Source](#setting-piece-source).                                                | `DB`                                                                 |                                                                        |
| `AP_PIECES_SYNC_MODE`              | `AP_PIECES_SYNC_MODE`: `NONE` disables syncing, `LOCAL_AUTO` syncs built pieces hourly, `OFFICIAL_AUTO` pulls from the registry URL.                                                                                         | `NONE`                                                                |
...
| `AP_TEMPLATES_SOURCE_URL`          | Optional endpoint Flowlytics - Workflow queries for template catalogs. Leave unset to disable remote templates entirely.                                                                                                                         | _unset_                         |                                                                        |
...
| `AP_APP_TITLE` | Initial title shown in the browser tab while loading the app | `Flowlytics - Workflow`  | `Flowlytics - Workflow`
| `AP_FAVICON_URL` | Initial favicon shown in the browser tab while loading the app | `/images/flowlytics-favicon.svg` | `/images/flowlytics-favicon.svg`

### OpenTelemetry Configuration

Flowlytics - Workflow supports both standard OpenTelemetry environment variables and vendor-specific configuration for observability and tracing.
