...
<Tip>
  These benchmarks are based on running Flowlytics - Workflow in `SANDBOX_CODE_ONLY` mode. This does **not** represent the performance of Flowlytics Cloud, which uses a different sandboxing mechanism to support multi-tenancy. For more information, see [Sandboxing](/install/architecture/workers#sandboxing).
</Tip>